...
const StarComponent: React.FC<StarComponentProps> = ({ rating }) => {
	const star = Array.from({ length: rating }, (_, index) => (
		<div
			className="inline-block bg-background-secondary border border-border rounded-full p-[9px]"
			key={index}>
			<svg
				className=""
				width="18"
	...
	return <div className="flex gap-2">{star}</div>;
};
